      ...
      if (!selectedCell && piece !== '') {
        ...
        if (selectedCell.rowIndex !== rowIndex || selectedCell.colIndex !== colIndex) {
          ...
          if (isValidMove(selectedCell.rowIndex, selectedCell.colIndex, rowIndex, colIndex, cells)) {
            // Se il pezzo è selezionato da un giocatore, controlla se l'altro giocatore ha scelto i pezzi in maiuscolo
            if ((myturn && piece === piece.toUpperCase()) || (!myturn && piece === piece.toLowerCase())) {
              ...
              if (cells[rowIndex][colIndex] !== '' && newCells[rowIndex][colIndex] !== '') {
              ...
              // Aggiungi qui una notifica o una gestione dell'errore
            }
          } else {
            setSelectedCell({ rowIndex, colIndex });
          }
    ...
    if (socket) {
      ...
      socket.on("current_turn", (currentTurn) => {
        ...
        if (moves.length % 2 === 0 || moves.length === 0) {
          ...
          setTurn(isMyTurn && moves[moves.length - 1].includes("+"));
        }
      });
